...
function getInv(almId) {
    if (!state.inventario.porAlmacen[almId]) {
        // Attempt to auto-create missing warehouses
...
function getProd(id, nombreAprox) {
    ...
    if (!p) {
        p = { id, nombre: nombreAprox || 'Prod Desconocido', canastasPrestadas: 0 };
...
function getCli(id, nombre) {
    ...
    if (!c && nombre) {
        c = { id: id || (Date.now() + Math.random().toString()), nombre, canastasPrestadas: 0, autocreado: true };
        state.clientes.push(c);
        console.log("¡Cliente faltante auto-creado a partir de operaciones!", nombre);
    }
    return c;
}
...
logs.forEach(act => {
    ...
    if (op === 'Recepción' || op === 'Recepción de Fruta') {
        ...
        const p = getProd(raw.productorId, act.detalle.includes('De: ') ? act.detalle.split('De: ')[1].split(',')[0] : null);
...
console.log("=== EXCELENCIA MATEMATICA ===");
...
const prodDeficit = state.productores.reduce((acc,p) => acc + (p.canastasPrestadas || 0), 0);
console.log("Suma Deuda Productores:", prodDeficit, "vs Global:", state.inventario.despachadasProductor);

const cliDeficit = state.clientes.reduce((acc,c) => acc + (c.canastasPrestadas || 0), 0);
console.log("Suma Deuda Clientes:", cliDeficit, "vs Global:", state.inventario.despachadasCliente);

console.log("\nClientes encontrados:");
state.clientes.forEach(c => console.log(`- ${c.nombre}: ${c.canastasPrestadas}`));

fs.writeFileSync('reconstructed_final.json', JSON.stringify(state, null, 2));
...
const req = https.request(options, (res) => {
    res.on('data', () => {});
    res.on('end', () => console.log('Recompilation Uploaded HTTP', res.statusCode));
});
req.write(payload);
req.end();
