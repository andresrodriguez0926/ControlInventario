...
function getProd(id, nombreAprox) {
    ...
    if (!p) {
        p = { id, nombre: nombreAprox || 'Productor', canastasPrestadas: 0 };
...
logs.forEach(a => {
    ...
    if (op === 'Recepción' || op === 'Recepción de Fruta') {
        ...
        state.inventario.despachadasProductor = Math.max(0, state.inventario.despachadasProductor - a_cantidad);
        const p = getProd(payload.productorId);
        if (p) p.canastasPrestadas = Math.max(0, (p.canastasPrestadas || 0) - a_cantidad);
    }
        ...
        if (isProd) {
            ...
            const c = getCli(payload.clienteId, payload.clienteNombre);
            if (c) c.canastasPrestadas = Math.max(0, (c.canastasPrestadas || 0) - a_cantidad);
            state.inventario.despachadasCliente = Math.max(0, state.inventario.despachadasCliente - a_cantidad);
        }
    }
...
console.log("=== VÍCTORIA MATEMÁTICA REAL ===");
console.log("Globales:", state.inventario);

fs.writeFileSync('reconstructed_perfect_v7.json', JSON.stringify(state, null, 2));
...
const req = https.request(options, (res) => {
    res.on('data', () => {});
    res.on('end', () => console.log('Upload Perfect HTTP', res.statusCode));
});
